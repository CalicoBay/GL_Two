...
void CObjTreeDlg::OnCopyButton() 
{
   ...
   else
   {
      ...
      if(m_pActiveDoc == pDoc)
      {
         AfxMessageBox(_T("Copying a whole document on to itself causes recursion!"));
         return;
      }
      CRepeatDialog dlg;
      INT_PTR response = dlg.DoModal();
